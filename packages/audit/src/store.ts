...
export function createAuditStore(pool: Pool, opts: AuditStoreOptions = {}): AuditStore {
  ...
  const rowToRecord = (row: unknown): AuditRecord => {
    ...
    return {
      ...
      ...(r["decision"] != null
        ? { decision: String(r["decision"]) as "allow" | "deny" }
        : {}),
    };
  };
  ...
  return {
    async append(input: AuditRecordInput): Promise<AuditRecord> {
      ...
      const scrubbedInput: AuditRecordInput = { ...input, attributes };

      const client = await pool.connect();
      try {
        await client.query("BEGIN");
        // Lock the chain tail so concurrent appends serialize.
        const tail = await client.query(
          `SELECT ${COLUMNS} FROM audit_log ORDER BY seq DESC LIMIT 1 FOR UPDATE`,
        );
        const prev = tail.rows.length > 0 ? rowToRecord(tail.rows[0]) : null;
        const record = appendRecord(prev, scrubbedInput, clock());
        await client.query(
    ...
    async verify(): Promise<VerifyResult> {
      // Verify the whole chain from genesis in seq order, paging to bound memory.
      let fromSeq = 1;
      const page = 5000;
      let all: AuditRecord[] = [];
      for (;;) {
