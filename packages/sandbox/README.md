...
- **Isolation is implemented, unit-tested, and adversarially reviewed --
  it is not yet CI-proven green.** This package's own unit tests
  ...
  unnested Linux host (or a CI runner that is one) -- this dev sandbox is
  not one (`bwrap --unshare-user ...` fails here with `setting up uid map:
  Permission denied`). The real isolation proof is
  `scripts/e2e-docker.sh`, wired into CI as the **`sandbox-boot-canary`**
  job (`.github/workflows/ci.yml`); a `sandbox-seccomp` job separately
  rebuilds and diffs the committed inner-filter BPF against its C source.
  Locally, `e2e-docker.sh` only reaches its **SKIP** path (preflight probe
  confirms this environment cannot create user namespaces, so it exits 0
  without ever building/running the real jail) -- it has **not yet run
  green on GitHub Actions**, because this branch has not yet been pushed.
  Treat this package's isolation claim as "implemented +
  unit-tested + CI-verified-on-push, pending first green
  `sandbox-boot-canary` CI run" -- not "proven," not "green CI," until that
  run is observed.
