...
- **Isolation is implemented, unit-tested, adversarially reviewed, AND now
  ...
  - **Known hardening gap (tracked):** bubblewrap does not mask its OWN freshly
    mounted `/proc`, so the jailed tool currently sees an unmasked `/proc`
    (`/proc/sched_debug`, `/proc/sysrq-trigger`, etc.). `systempaths=unconfined`
    does not cause this (it only unmasks the trusted supervisor's `/proc`); it
    is bwrap's default once proc-mounting works, and is to be closed by masking
    those paths inside the bwrap invocation.
