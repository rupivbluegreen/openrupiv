...
- **Isolation is implemented, unit-tested, and adversarially reviewed --
  but it is NOT empirically proven, and the CI job that was meant to prove
  it SKIPS rather than proving anything.** This package's own unit tests
  (`test/*.test.ts`) never invoke real `bwrap`; they inject a fake
  jail-runner, because creating Linux user namespaces requires a real,
  unnested Linux host -- this dev sandbox is not one (`bwrap --unshare-user
  ...` fails here with `setting up uid map: Permission denied`). The real
  isolation proof is `scripts/e2e-docker.sh`, wired into CI as the
  **`sandbox-boot-canary`** job (`.github/workflows/ci.yml`).
  **Confirmed on PR #11's first CI run (GitHub Actions run 29152854424):
  the `sandbox-boot-canary` job also hits its SKIP path on GitHub Actions
  `ubuntu-latest` runners** -- the preflight probe fails with `bwrap:
  loopback: Failed RTM_NEWADDR: Operation not permitted`, i.e. a container
  on a standard hosted runner cannot bring up the loopback interface inside
  the jail's nested user+net namespace. The job therefore **exits 0 without
  ever building or running a real jail**, so its green check is a vacuous
  skip, NOT a verification. A green `sandbox-boot-canary` today proves
  nothing about isolation.
  - **What IS genuinely CI-verified:** the separate `sandbox-seccomp` job
    rebuilds the inner-filter BPF from its C source on ubuntu:24.04 (real
    libseccomp toolchain, no user namespace needed) and diffs it against the
    committed `tool.bpf` -- that job really runs and passed (`check-seccomp-bpf:
    OK`).
  - **What is NOT proven anywhere available:** the kernel-level isolation
    itself (network-egress block, fs-escape block, rlimit enforcement,
    SIGSYS on blocked syscalls). It has only been verified by construction +
    adversarial review + fake-jail unit tests. Running the real proof
    requires a `bwrap`-capable environment neither this dev box nor a
    standard hosted GitHub Actions runner provides -- a privileged container,
    a self-hosted runner, or a bare-metal/VM Linux host with unprivileged
    user namespaces and in-container loopback setup permitted. **Until the
    real e2e is observed green on such a host, treat the isolation claim as
    "implemented + reviewed, NOT empirically proven."**
