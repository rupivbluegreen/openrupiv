...
| Sandboxed code execution | 🚧 `packages/sandbox` built, unit-tested (86 tests), and adversarially reviewed: per-execution bubblewrap jails (unprivileged user namespaces), boot canary, resource limits, seccomp — [ADR-0007](docs/adr/0007-agent-sandbox-bubblewrap-sidecar.md), status: proposed, human-only review path, **not yet human-reviewed/merged**. `createSidecarSandbox` implements `ToolSandbox` and `openrupiv new` generates the `sandbox` Compose service, which starts and passes its own boot canary. The inner seccomp filter's BPF is genuinely CI-verified (the `sandbox-seccomp` job rebuilds it from C on ubuntu:24.04 and diffs the committed artifact — passed). But **real kernel-level isolation (network egress block, fs escape block, rlimit enforcement) is NOT empirically proven**: the `sandbox-boot-canary` e2e job SKIPS on GitHub Actions `ubuntu-latest` too (confirmed on PR #11 run 29152854424 — `bwrap: loopback: Failed RTM_NEWADDR: Operation not permitted`; a standard hosted runner can't bring up loopback in the jail's nested namespace), so it exits 0 without running a real jail — its green check is a vacuous skip, not a verification. Isolation is verified only by construction + adversarial review + fake-jail unit tests; running the real proof needs a `bwrap`-capable host (privileged container, self-hosted runner, or bare-metal/VM Linux) that is not yet available in CI. **Not yet wired into any generated app's runtime deployment** — `packages/runtime`'s `bin/serve.mjs` constructs no `AgentRuntime`/`ToolSandbox`, the generated runtime service's Compose env carries no `SANDBOX_TOKEN`, and v1 ships zero concrete Python `RegisteredTool` implementations — so there is nothing yet for a real agent task to sandbox end-to-end |
| Rate limits | 📋 M3 |
